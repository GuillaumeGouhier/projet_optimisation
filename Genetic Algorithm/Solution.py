...
## TODO: Check If Solution Is Viable
    def updatePossible(self):
        ...
        liste_candidate = [i for i in range(len(self.genoa)) if self.genoa[i] == 1]
        checkLen = ()
        for i in self.genoa:
    ...
    def mutate(self, proba):
        self.genoa = list(map(lambda x : 1 - x if random.random() <= proba else x, self.genoa))
        return 0

    def repair (self, data):
        # Repair phase to ensure every child solution can live
        invites_candidate = [i for i in range(len(self.genoa)) if self.genoa[i] == 1]
        # Map invite to score
        ...
        while self.isPossible == False:
            self.genoa[invites_candidate[scores[i][1]]] = 0
            i=i+1
